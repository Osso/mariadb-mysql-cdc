...
rolls back the target transaction; ignored duplicate `1062` row changes are
logged and committed without ledger persistence. Guarded observation upserts are
idempotent, and the live target checkpoint does not advance for durable
constraint conflicts. The admin-bootstrapped
...
- Native ROW `INSERT` and `UPDATE` events honor the policy. With
  `ignore-duplicate`, a MySQL `1062` is logged as skipped without creating a
  durable conflict record, so the target transaction and live checkpoint can
  commit. With the default `error` policy, the duplicate fails the row event,
  rolls back the target transaction, and leaves the checkpoint unchanged.
