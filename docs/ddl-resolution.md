...
database harness is in progress. The matrix distinguishes semantic unit proof
from real MariaDB/MySQL replay proof; neither is a deployment claim. Historical
details below describe additional narrow slices, including
...
combination support. Current proof is semantic-unit coverage; real separate-
event success and same-event fail-closed scenarios are in progress.

For lifecycle DDL, ordinary comments are inert syntax. This includes MariaDB's
server-rewritten `DROP TABLE \`name\` /* generated by server */` form, whose
identifier is normalized into deterministic target SQL. Executable/version
comments and optimizer hints remain blocked. Target inventory initializes
`SET SESSION information_schema_stats_expiry=0`: cached `AUTO_INCREMENT`
metadata can survive `TRUNCATE` and a stream restart, while uncached engine
metadata is required for restart reconciliation.
