...
`DATETIME`, signed `TINYINT(1)`, `TINYINT(1) UNSIGNED`, `SMALLINT UNSIGNED`, or `FLOAT UNSIGNED`; plus the exact
...
precision, noncanonical `TINYINT` display widths, `SMALLINT` display width, `FLOAT` parameters, and every other guarded
...
MariaDB-syntax `ADD INDEX` normalized to the same AST, or `ADD UNIQUE KEY` clauses
are admitted. Signed `TINYINT(1) NOT NULL DEFAULT 0` renders as MySQL `TINYINT NOT
NULL DEFAULT 0`, preserving the signed `-128..127` range; its `UNSIGNED` variant
remains `TINYINT UNSIGNED`. Multiple admitted clauses render in source order as deterministic MySQL
...
### Current common-operation constraints

For source `ALTER TABLE t ALTER COLUMN body SET DEFAULT '{}'` on `TEXT` or
`MEDIUMTEXT`, MySQL 8.4 rejects the direct expression form
`ALTER COLUMN body SET DEFAULT (_utf8mb4'{}')` with error 1101. The translator
uses one target-derived `MODIFY COLUMN` definition instead, retaining type,
nullability, character set, collation, comment, and position while rendering
the expression default. It requires an ordinary existing column with fully
modeled attributes; there is no fallback execution path. This workaround is
retired only when a real-engine probe proves direct expression `SET DEFAULT`
works with the required metadata preservation.

`ALTER TABLE t ADD COLUMN c ..., ALTER COLUMN c SET DEFAULT ...` is a known
**required** gap. MySQL rejects the same-event dependency with error 1054, even
for a `CONCAT` expression probe. Translation fails before target DDL: the
journal remains `translation_pending` and the checkpoint remains unchanged.
The ordered two-event sequence is supported. Ordered-DDL replay is required to
handle the single-event form; this is not a product exclusion or a claim of full
combination support. Real replay proves separate-event success and same-event
fail-closed preservation: the journal stays `translation_pending` and no target
DDL runs.

For lifecycle DDL, ordinary leading or trailing comments are inert syntax. This
includes MariaDB's server-rewritten trailing `DROP TABLE \`name\` /* generated by
server */` form, whose identifier is normalized into deterministic target SQL.
Executable/version comments and optimizer hints remain blocked. Real lifecycle
replay covers rename with generated/named and inbound/self foreign keys, indexes,
and triggers; truncate row clearing/auto-increment reset and restart; drop and
restart; and absent `IF EXISTS` no-op followed by DML. Target inventory
initializes `SET SESSION information_schema_stats_expiry=0`: cached
`AUTO_INCREMENT` metadata can survive `TRUNCATE` and a stream restart, while
uncached engine metadata is required for restart reconciliation.
