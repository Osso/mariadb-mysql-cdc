...
- [x] Translate `ENUM` columns, preserving the case of their values; `SET` columns are still rejected explicitly. The staged convergence limit for existing target ENUMs is defined in [Unified source-authoritative synchronization](unified-sync.md#schema-and-progress-contracts): automatic convergence permits only source-proven label appends in existing order.
...
- [x] Preserve valid target FK/CHECK constraints for unchanged, additive column/index, and safe source-proven ENUM-append prerequisites. A destructive prerequisite retains the existing drop/re-add path; this is not universal zero-DDL.
