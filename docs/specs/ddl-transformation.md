...
(_utf8mb4'...')`. It is not a runtime fallback. Retire it only after a real
MySQL engine probe shows the direct `SET DEFAULT` expression form works and
preserves the required metadata; then replace the renderer rather than keeping
both paths. Until then, error 1101 makes this single renderer the required
compatibility path, not an optional alternate.
