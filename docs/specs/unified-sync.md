...
- [x] Preserve existing target foreign keys and CHECK constraints when prerequisite structural convergence is empty or consists only of additive columns, indexes, or a safe source-proven ENUM append. This avoids routine constraint rebuilds for unchanged and additive tables.
- [x] For destructive prerequisite structural changes, retain the existing constraint-drop behavior; this is not universal zero-DDL or selective destructive dependency analysis.
