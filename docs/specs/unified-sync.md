...
- [x] Invoke bounded row workers between the two schema stages. For tables without completed legacy `rows` progress, run source-authoritative `insert_missing`, `update_divergent`, then `delete_extras` phases; schedule ordinary inserts/updates parent-first and deletes child-first from current same-schema FK inventory. A restrictive referenced-key transition coordinates its affected descendants child-first inside the same locked target transaction. Only unrelated tables share a bounded batch. Before a worker reads source rows, it locks the selected same-schema FK component needed for its mutations through mutation, commit, and phase-progress persistence. An unselected inbound child does not expand or reject a parent-only run; its existing FK remains enforced and rejects an incompatible selected-parent mutation.
...
- [x] On a restrictive referenced-key parent update rejected with MySQL `1217` or `1451`, keep constraints enabled and, inside the component `WRITE` lock and the same target transaction, detach the affected target-dependent subtree child-first, update the root parent, and restore only current source rows parent-first. Strict writes read back every detached/restored row; source-absent dependent rows remain deleted. A new child insert rejected with `1216` or `1452` resolves its required source parent recursively in that transaction before one bounded retry. This transition never drops or disables a foreign key.
- [x] When a `1062` unique owner is source-absent but still referenced on target, detach its target descendants child-first, delete that owner, insert or update the intended source owner, and restore only current source descendants parent-first in the same locked transaction. Source-present owner reconciliation retains the existing strict path. Both forms are source-authoritative and emit the reconciliation audit only after commit.
- [x] Prove restrictive referenced-key transition, a changed-parent new-child insert, source-absent unique-owner replacement with `RESTRICT` descendants, rollback/resume, and injected phase-cursor persistence failure against disposable MariaDB/MySQL. Evidence: `/tmp/claude/cdc-fk-transition-integration-batch.log` and `/tmp/claude/cdc-source-absent-owner-green.log`.
- [x] Retain at most 64 MiB of decoded payload per dependent page, except one oversized row. Transition work retains primary-key identities for all affected rows, so its memory is O(affected rows); the page budget is not a total process-memory bound. There is no arbitrary one-million-key limit and no global source snapshot.
...
- [x] Before rows, remove only target FK/CHECK definitions absent from, or semantically different from, source evidence. Retain valid semantic equivalents even when target FK/CHECK names differ. This removes row-blocking obsolete constraints without routine rebuilds.
- [x] Preserve valid target FK/CHECK definitions when prerequisite structural convergence is empty or consists only of additive columns, indexes, or a safe source-proven ENUM append. Destructive prerequisite structural changes retain the existing dependency-sensitive drop path; this is not universal zero-DDL or selective destructive dependency analysis.
...
- [x] Reuse the final-constraint stage after row work to add missing definitions and fail closed on remaining structural drift. Equal FK/CHECK semantics retain an existing target name; prerequisite removal uses an actual target name, while a new constraint uses the mapped target name.
- [x] Bootstrap an additive target table named `<progress-table>_phases`, keyed by unchanged run ID, table name, and explicit mutation phase. The default table DDL and exact `cdc_stream` phase read/write grant are in [`sync-phase-progress-bootstrap.sql`](../sync-phase-progress-bootstrap.sql). Current runtime also executes idempotent `CREATE TABLE IF NOT EXISTS` before incomplete row work, so the bootstrap does not remove its existing `CREATE ON cdc.*` requirement; no new broad grant is claimed here. A completed legacy `rows` record is reused unchanged; incomplete legacy cursors do not seed phase cursors. The aggregate legacy `rows` record is written only after all three phases finish.
- [x] Prove an injected phase-cursor write failure after a committed restrictive transition: same-ID resume reaches the desired state without repeating the restoration trigger. The phase cursor still uses a separate post-commit target connection; this proof does not claim one atomic transaction across target mutation and cursor persistence.
- [x] Prove disposable MariaDB/MySQL valid FK/CHECK preservation without ALTER privilege; parent-only selection; parent/child/grandchild key transition, reparent, and new-child ordering; semantic FK-name preservation; source-absent owner replacement; rollback/resume; and legacy all-`rows`-complete reuse without phase-table access. Evidence: `/tmp/claude/sync-constraints-preserved-integration4.log`, `/tmp/claude/cdc-fk-transition-integration-batch.log`, `/tmp/claude/cdc-source-absent-owner-green.log`, and `/tmp/cdc-legacy-complete-07c9038.log`. No deployment has occurred.

**Known constraint-preserving limits.** Cyclic/self-referential FK scheduling and destructive structural changes remain fail-closed or use the existing destructive schema path. They do not authorize routine FK drops.
...
- `src/sync/phase_progress.rs` — additive explicit phase cursor storage; it does not alter legacy `cdc.sync_runs` rows. Deployment bootstraps `<progress-table>_phases` before runtime.
- `src/sync/chunk.rs` — locked source/target chunk mutation and run/table progress boundary.
- `src/sync/mysql.rs` and `src/sync/mysql/fk_transition.rs` — source, component-locked target-session, separate progress-store adapters, strict unique-owner reconciliation, and coordinated FK transitions.
...
- [ ] Run full-project tests, Clippy without warning suppression, and final integration verification. The September 9, 2026 redesign has targeted disposable phase proof but no deployment.
