...
- [x] Bootstrap an additive target table named `<progress-table>_phases`, keyed by unchanged run ID, table name, and explicit mutation phase. The default bootstrap and exact `cdc_stream` table grant are in [`sync-phase-progress-bootstrap.sql`](../sync-phase-progress-bootstrap.sql); runtime does not create it or require schema-wide `cdc.*` privileges. A completed legacy `rows` record is reused unchanged; incomplete legacy cursors do not seed phase cursors. The aggregate legacy `rows` record is written only after all three phases finish.
