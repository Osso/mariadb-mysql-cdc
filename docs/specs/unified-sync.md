...
- [ ] Prove the integrated phase executor against disposable MariaDB/MySQL endpoints: valid existing FK/CHECK preservation; parent/child insert, reparent, and delete ordering; restart after each phase; phase/legacy progress consistency; and no `ALTER TABLE` for unchanged schemas. Phase cursor persistence currently uses a separate post-commit target connection, so crash/retry behavior must be proven rather than described as atomic. No deployment has occurred.
