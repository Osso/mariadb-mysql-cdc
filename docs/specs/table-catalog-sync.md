...
- [x] Write deterministic, pretty JSON syncable and non-syncable catalogs without using `COUNT(*)`; reject output paths that resolve to the same filesystem destination before creating or overwriting either file, including lexical aliases, symlinks, and hardlinks.
...
- [x] Include ordered primary-key columns, writable sync columns, estimated source rows, and the union of applicable source and target FK parent dependencies. Target-only local FKs gate child scheduling exactly like source FKs.
...
- [x] Use deterministic versioned run IDs that length-frame and hexadecimal-encode every byte of the full `(prefix, target database, table)` tuple. Require a non-empty prefix and reject any generated ID over 128 bytes before execution; delimiter placement inside any component must not create a collision.
