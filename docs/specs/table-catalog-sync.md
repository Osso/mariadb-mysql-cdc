...
- [x] Use deterministic per-table run IDs in the exact form `<run-id-prefix>-<normalized-target-database>-<table>`, where the database component preserves ASCII letters, digits, `_`, and `-` and hex-escapes every other UTF-8 byte. Require a non-empty prefix and reject any generated ID over 128 bytes before execution; the same prefix/table pair must not collide across target databases.
