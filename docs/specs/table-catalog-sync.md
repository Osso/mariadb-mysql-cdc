...
- [x] Regenerate syncable catalogs after any change to primary-key ordering semantics. Reject catalogs generated before persisted `primary_key_ordering` support instead of treating their missing metadata as native ordering. Use the regenerated catalog with a fresh `--run-id-prefix`; do not resume or trust existing ENUM-key child run IDs from the prior specification. Existing all-native direct runs retain their prior immutable run JSON and remain resumable.
