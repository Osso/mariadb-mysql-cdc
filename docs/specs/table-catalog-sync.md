...
- [x] Apply catalog tables through the existing table-sync engine without a `max_deletes` limit or delete-safety gate. `sync-catalog` must apply every target orphan deletion planned by the catalog comparison; it exposes no deletion-limit override because catalog execution is already bounded by the catalog-planned orphan set.
- [x] Keep deletion controls for other commands separate: direct `sync-table` apply mode requires its explicit bounded `--max-deletes` allowance, and `repair-drift` apply mode requires an explicit bounded `--max-deletes` allowance. Neither command inherits `sync-catalog`'s catalog-planned deletion contract.
- [x] Use deterministic fixed-length `catalog-v2-<SHA-256 hex>` run IDs. Hash the injective length-framed byte tuple `(prefix, target database, table)`, require a non-empty prefix, and keep every generated ID within the 128-byte progress-column limit while preserving prefix/database/table identity.
