...
- [x] Write deterministic, pretty JSON syncable and non-syncable catalogs without using `COUNT(*)`; reject output paths that resolve to the same filesystem destination before creating or overwriting either file, including lexical aliases, intermediate-symlink-plus-`..` aliases, symlinks, and hardlinks, and fail closed on symlink cycles. For partly nonexistent paths, canonicalize the longest existing physical ancestor before normalizing the remaining suffix.
