...
- [x] Derive the durable run ID from the prefix alone and persist staged progress by `(run_id, stage, table_name)` in `cdc.sync_runs`; schema prerequisites, locked source-authoritative row chunks, and final constraints are owned by unified sync. Changed invocation settings or catalog contents require no authorization or progress migration.
