...
- [x] Apply catalog tables through the existing table-sync engine with an explicit bounded `--max-deletes` allowance required for every `sync-catalog` invocation. The bound is passed to each child table-sync run and enforced before target-orphan deletion mutates a violating chunk.
- [x] Keep deletion controls for other commands separate: direct `sync-table` apply mode and `repair-drift` apply mode also require explicit bounded `--max-deletes` allowances; none inherits another command's bound.
...
- [x] Use deterministic fixed-length `catalog-v2-<SHA-256 hex>` run IDs. Hash the injective length-framed byte tuple `(prefix, target database, table)`, keep every generated ID within the 128-byte progress-column limit while preserving prefix/database/table identity, and include the explicit `max_deletes` bound in each child's immutable run specification.
