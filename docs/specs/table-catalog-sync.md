...
- [x] Limit table-sync capacity to sixteen target-server slots shared by direct `sync-table` and `sync-catalog` workers. New admission and slot reservations are serialized and scoped by the lower-cased target host plus port, so databases on the same server share capacity. Each worker reserves one slot plus a database/table-specific lock whose identifier components are injectively length-framed and hexadecimal-encoded. Legacy run-lock accounting and same-table detection inspect only the configured progress table. Within that table, identity comes from the immutable run specification's `scope.target_database` and `table.name`, not `table_name` alone, so same-named tables in different target databases neither block nor complete each other's catalog dependencies. A held legacy run-ID advisory lock for the requested same database/table excludes that table even without a table reservation. Rows in `running`, `complete`, or `error` with a held legacy run-ID advisory lock but no table reservation consume equivalent capacity. Before counting capacity, and under the admission lock, mark every `running` row whose `GET_LOCK(SHA2(run_id,256))` is unheld as `error` with `run abandoned: worker exited without releasing its run lock`, reporting the count as `cdc_table_sync_runs_reaped`. A run takes that lock before it writes its row and holds it for the process lifetime, so an unlocked `running` row is a dead worker, never a starting one; leaving it `running` made the progress table unreadable as status. The recorded error keeps the resume path intact, because a later run with the same immutable specification claims the failed row and continues from its stored key. Ignore stale unlocked `running` and `error` rows before parsing immutable specifications. Malformed lock-active rows fail closed. Always parse an expected completed child and require its immutable specification to match exactly before treating it as terminal.
...
- [x] Regenerate syncable catalogs after any change to primary-key ordering semantics. Catalogs generated before persisted `primary_key_ordering` support may decode through the legacy default but are not safe execution inputs: they can make pre-change completed runs appear terminal and cannot produce an immutable run specification matching the current worker. Use the regenerated catalog with a fresh `--run-id-prefix`; do not resume or trust existing child run IDs from the prior specification.
...
- More than eight simultaneous table syncs, including externally active runs.
