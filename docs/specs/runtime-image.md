...
- [x] Before creating a Job that can execute incomplete unified-sync rows, bootstrap the selected `<progress-table>_phases` table and grant only `SELECT`, `INSERT`, and `UPDATE` on it to that Job's target account. The default `cdc.sync_runs_phases` SQL is [`sync-phase-progress-bootstrap.sql`](../sync-phase-progress-bootstrap.sql). Image deployment does not create it, but current Job runtime still issues idempotent `CREATE TABLE IF NOT EXISTS` and retains its existing `CREATE ON cdc.*` requirement; this spec does not broaden that grant.
