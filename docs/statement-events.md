...
live sample passed with no pod restarts or quarantines. Source literals use
immutable tag-1 `SQL_MODE` context for
standard and
...
- the production-observed unqualified multi-clause `ALTER TABLE` form with
  ...
  proven a no-op when every one exists exactly; and `ADD CONSTRAINT <name> CHECK
  (...)` under the bounded CHECK grammar in
