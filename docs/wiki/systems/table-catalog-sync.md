...
Each child uses a fixed-length `catalog-v2-<SHA-256 hex>` run ID derived from the injective length-framed byte tuple `(prefix, target database, table)`, applies with `max_deletes=0`, and persists progress in the configured run table. Hashing preserves tuple identity while keeping long valid names within the 128-byte progress column. An interrupted child resumes only with the same immutable specification. An expected completed child is parsed and terminal only when its specification matches exactly. Stale unlocked running/error rows are ignored before spec parsing; malformed lock-active rows fail closed.
