...
BEGIN
    ...
    FOR UPDATE;
    IF locked_source_rows<>95 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Active source entitlement count changed; expected 95';
    END IF;
    ...
    FROM release_move_purchase_exclusions exclusion_row
    LEFT JOIN releases_users_purchases source_rup ON source_rup.id=exclusion_row.rup_id
    LEFT JOIN releases_users_purchases destination_rup ON destination_rup.id=exclusion_row.destination_rup_id
    WHERE source_rup.id IS NULL
       OR source_rup.user_id<>exclusion_row.user_id
       OR source_rup.release_id<>exclusion_row.from_release_id
       OR source_rup.is_active<>1
       OR destination_rup.id IS NULL
       OR destination_rup.user_id<>exclusion_row.user_id
       OR destination_rup.release_id<>exclusion_row.to_release_id
       OR destination_rup.is_active<>1;
    IF invalid_rows<>0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='Destination-duplicate exclusions changed from manifest';
    END IF;
