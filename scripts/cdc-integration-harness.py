...
SCENARIOS = (
    ...
    ScenarioSpec("users-update-current-mismatch", True),
    ScenarioSpec("users-update-source-missing", True),
...
class Harness:
    ...
    def run_users_update_snapshot_replay(self, scenario: str) -> None:
        assert self.source and self.target
        schema = """
            CREATE TABLE users (
                id BIGINT NOT NULL PRIMARY KEY,
            ...
            CREATE TABLE user_update_markers (id INT PRIMARY KEY) ENGINE=InnoDB;
        """
        for endpoint in (self.source, self.target):
            self.admin_sql(endpoint, schema)
        self.admin_sql(
            self.source,
            "INSERT INTO users VALUES (1,'original','current'),(2,'other','owner');",
        )
        start = self.coordinate()
        self.write_checkpoint(start)
        ...
        self.admin_sql(
            self.source,
            "START TRANSACTION; INSERT INTO user_update_markers VALUES (1); "
            "UPDATE users SET name='reassigned' WHERE id=1; "
            "INSERT INTO user_update_markers VALUES (2); COMMIT;",
        )
        historical_stop = self.coordinate()
        self.admin_sql(
            self.source,
            "START TRANSACTION; UPDATE users SET name='current-name' WHERE id=1; "
            "UPDATE users SET name='reassigned' WHERE id=2; COMMIT;",
        )
        final_stop = self.coordinate()
        if scenario == "users-update-source-missing":
            self.admin_sql(self.source, "DELETE FROM users WHERE id=1;")
        payload = (
            "divergent" if scenario == "users-update-current-mismatch" else "current"
        )
        self.admin_sql(
            self.target,
            "INSERT INTO users VALUES "
            f"(1,'current-name',{sql_literal(payload)}),(2,'reassigned','owner');",
        )
        before = self.admin_query(self.target, "SELECT * FROM users ORDER BY id;")
        positive = scenario == "users-update-snapshot-ahead"
        boundaries = [historical_stop, final_stop] if positive else [historical_stop]
        binary = self._stream_binary(None)
        for stop in boundaries:
            args = self._stream_args(binary, start, stop, None, 0)
            ...
            if (
                self.admin_query(self.target, "SELECT * FROM users ORDER BY id;")
                != before
            ):
                raise HarnessError(f"{scenario}: current target users changed")
            markers = self.admin_query(
                self.target, "SELECT id FROM user_update_markers ORDER BY id;"
            ).splitlines()
            checkpoint = self.checkpoint()
            self.assert_admin_sql_rejected(
                self.target,
                "INSERT INTO users VALUES (99,'reassigned','probe');",
                "Duplicate entry",
            )
            if (
                self.admin_query(self.target, "SELECT * FROM users ORDER BY id;")
                ...
                raise HarnessError(f"{scenario}: uniqueness probe changed users")
            if positive:
                if result.returncode and (markers or checkpoint != checkpoint_before):
                    raise HarnessError(
                        f"{scenario}: rejected replay committed transaction state"
                    )
                require_success(result, scenario)
                if markers != ["1", "2"] or (
                    checkpoint.get("source_file"),
                    int(checkpoint.get("source_position", 0)),
                ) != (stop.file, stop.position):
                    raise HarnessError(
                        f"{scenario}: transaction boundary not committed"
                    )
            else:
                if (
                    result.returncode == 0
                    or "1062" not in result.stdout + result.stderr
                ):
                    raise HarnessError(f"{scenario}: expected duplicate-key rejection")
                if markers or checkpoint != checkpoint_before:
                    raise HarnessError(f"{scenario}: failed transaction advanced state")
            start = stop
        print(
            f"users_update_replay_ok scenario={scenario} snapshot=unchanged unique=enforced"
        )
...
"""
    ...
    def journal_full_row(self) -> dict[str, str]:
        ...
        if len(rows) != 1:
            raise HarnessError(
                f"expected exactly one immutable journal row, got {output}"
            )
        if len(rows[0]) != 15:
            raise HarnessError(
                f"immutable journal row column mismatch count={len(rows[0])} output={output!r}"
            )
        names = [
            "source_identity",
            "source_server_id",
    ...
    def run_scenario(self, scenario: str) -> None:
        ...
        elif scenario.startswith("users-update-"):
            self.run_users_update_snapshot_replay(scenario)
