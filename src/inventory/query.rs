...
/// Restricts a schema-wide inventory query to one table. Verification of a single table must
/// not read every table's metadata.
pub(crate) fn table_filter(column: &str, table: Option<&str>) -> String {
    match table {
        Some(table) => format!(" AND {column} = {}", quote_sql_string(table)),
        None => String::new(),
    }
}

pub(crate) fn tables_query(schema: &str, table: Option<&str>) -> String {
    format!(
        ...
        quote_sql_string(schema),
        table_filter("TABLE_NAME", table)
    )
}

pub(crate) fn columns_query(schema: &str, table: Option<&str>) -> String {
    format!(
        "SELECT TABLE_NAME, COLUMN_NAME, ORDINAL_POSITION, COLUMN_TYPE, DATA_TYPE, IS_NULLABLE, CHARACTER_SET_NAME, COLLATION_NAME, COLUMN_DEFAULT, EXTRA, COLUMN_COMMENT, GENERATION_EXPRESSION, COLUMN_DEFAULT IS NULL FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = {}{} ORDER BY TABLE_NAME, ORDINAL_POSITION",
        quote_sql_string(schema),
        table_filter("TABLE_NAME", table)
    )
}

pub(crate) fn primary_keys_query(schema: &str, table: Option<&str>) -> String {
    format!(
...
pub(crate) fn foreign_keys_query(schema: &str, table: Option<&str>) -> String {
    format!(
        "SELECT TABLE_NAME, CONSTRAINT_NAME, COLUMN_NAME, ORDINAL_POSITION, REFERENCED_TABLE_SCHEMA, REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = {} AND REFERENCED_TABLE_NAME IS NOT NULL{} ORDER BY TABLE_NAME, CONSTRAINT_NAME, ORDINAL_POSITION",
        quote_sql_string(schema),
        table_filter("TABLE_NAME", table)
    )
}

pub(crate) fn canonical_foreign_keys_query(schema: &str, table: Option<&str>) -> String {
    format!(
        ...
        quote_sql_string(schema),
        table_filter("k.TABLE_NAME", table)
    )
}
