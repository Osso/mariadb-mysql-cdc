    ...
    pub(crate) fn restore_displaced_owner_and_insert_transactionally(
        ...
        let result = (|| {
            ...
            let dependencies_before =
                dependency_fingerprint(executor, table, displaced_source, missing_source)?;
...
fn dependency_fingerprint(
    ...
    let metadata_sql = format!(
        "SELECT TABLE_NAME,CONSTRAINT_NAME,COLUMN_NAME,REFERENCED_COLUMN_NAME,ORDINAL_POSITION \
    ...
    constraints
        .into_iter()
        .map(|((child_table, constraint), columns)| {
            let predicate =
                child_identity_predicate(&columns, &[displaced_source, missing_source])?;
            let sql = format!(
            ...
            let mut rows = executor
                ...
                .map_err(target_repair_error)?;
            rows.sort();
            Ok((child_table, constraint, rows))
        })
        .collect()
}

type ForeignKeyColumns = BTreeMap<(String, String), Vec<(String, String)>>;
...
fn group_foreign_keys(rows: Vec<Vec<Option<String>>>) -> Result<ForeignKeyColumns, TableSyncError> {
    ...
    for row in rows {
        ...
        let child_table = required_field(&row[0], "child table")?;
        let constraint = required_field(&row[1], "constraint")?;
        let child_column = required_field(&row[2], "child column")?;
        let parent_column = required_field(&row[3], "parent column")?;
        constraints
            .entry((child_table, constraint))
